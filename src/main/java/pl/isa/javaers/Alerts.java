...
//                System.out.println("jsonAlertsArray : " + jsonAlertsArray);
    ...
    public  void deleteAlert() {
        int alertID;
        System.out.println("Podaj Id alertu:");
        alertID = scanner.nextInt();
        try {
            if (alerts.contains(alertID)) {
                //Wydaje mi się, że będzie łatwiej gdy dla kazdego alertu będziemy tworzyć osobny plik bo chyba nie da się usunąc konkretnej linijki w pliku

                System.out.println("alert został skasowany");
            } else {
                System.out.println("Nie ma takiego alertu");
            }
        }catch (Exception e){
            e.printStackTrace();
        }
